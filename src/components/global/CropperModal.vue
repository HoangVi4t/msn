...
let fileInput = ref(null) // tham chiếu đến DOM input
let cropper = ref(null) // tham chiếu DOM croper
let uploadedImage = ref(null) // url hình ảnh cần crop
let croppedImageData = { // dữ liệu khi sẽ nhận sau khi crop img
  file: null,
  imageUrl: null,
  height: null,
  width: null,
...
const getUploadedImage = (e) => {
  const file = e.target.files[0] // truy cập tệp đã chọn
  uploadedImage.value = URL.createObjectURL(file) //lấy đường dẫn của file
}

const crop = () =>{ // crop images
  const {coordinates,canvas} = cropper.value.getResult() //getResult() được gọi để lấy ra các thông tin về vị trí và kích thước của vùng được cắt từ ảnh
                                                            // và gán vào 2 biến coordinates và canvas
  ...
  emit('croppedImageData', croppedImageData) // phát ra event đến các component khác
